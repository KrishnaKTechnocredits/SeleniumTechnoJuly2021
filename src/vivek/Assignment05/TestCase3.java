/*Test Case 3 - 
Naivgate to "http://automationbykrishna.com/#"
Navigate to Basic Elements tab
Navigate to DIFFERENT EXAMPLES OF ALERTS section
Click on Javascript Confirmation button
Validate the alert message and accept the alert.
Validate the confirmation message displayed below the Javascript Confirmation button.
Click on Javascript Confirmation button
Dismiss the alert.
Validate the confirmation message displayed below the Javascript Confirmation button.*/

package vivek.Assignment05;

import org.openqa.selenium.Alert;
import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.chrome.ChromeDriver;

public class TestCase3 {

	static WebDriver openBrowser(String url) {
		System.setProperty("webdriver.chrome.driver", "./resources/chromedriver.exe");
		WebDriver driver = new ChromeDriver();
		driver.manage().window().maximize();
		driver.get(url);
		return driver;
	}

	static void validateJavaScriptConfirmationButton() throws InterruptedException {
		System.out.println("Step 1: Open the browser");
		WebDriver driver = openBrowser("http://automationbykrishna.com/");
		System.out.println("Step 2: Click on Basic Elements");
		driver.findElement(By.linkText("Basic Elements")).click();
		Thread.sleep(2000);
		System.out.println("Step 3: Click on JavaScript Confirmation button");
		driver.findElement(By.id("javascriptConfirmBox")).click();
		System.out.println("Step 4: Validate the Alert message");
		Alert alert = driver.switchTo().alert();
		String expectedAlertMessage = "Are you are doing your homework regularly, Press Okay else Cancel!!";
		String actualAlertMessage = alert.getText();
		if (actualAlertMessage.equals(expectedAlertMessage))
			System.out.println(actualAlertMessage + " is the expected message to be displayed");
		else
			System.out.println(actualAlertMessage + " is not the expected message");
		System.out.println("Step 5: Accept the alert");
		alert.accept();

		System.out.println(
				"Step 6: Validate the confirmation message displayed below the Javascript Confirmation button");
		String expectedConfirmationMessage = "You pressed OK!";
		String actualConfirmationMessage = driver.findElement(By.id("pgraphdemo")).getText();
		if (actualConfirmationMessage.equals(expectedConfirmationMessage))
			System.out.println(actualConfirmationMessage + " is the expected confirmation to be displayed");
		else
			System.out.println(actualConfirmationMessage + " is not the expected confirmation");
		System.out.println("Step 3: Click on JavaScript Confirmation button again");
		driver.findElement(By.id("javascriptConfirmBox")).click();
		System.out.println("Step 5: Dismiss the alert");
		alert.dismiss();
		System.out.println(
				"Step 6: Validate the confirmation message displayed below the Javascript Confirmation button");
		expectedConfirmationMessage = "You pressed Cancel!";
		actualConfirmationMessage = driver.findElement(By.id("pgraphdemo")).getText();
		if (actualConfirmationMessage.equals(expectedConfirmationMessage))
			System.out.println(actualConfirmationMessage + " is the expected confirmation to be displayed");
		else
			System.out.println(actualConfirmationMessage + " is not the expected confirmation");
	}

	public static void main(String[] args) throws InterruptedException {
		validateJavaScriptConfirmationButton();
	}
}
